...
export class Backend implements backendInterface {
    ...
    async submitBooking(arg0: Booking): Promise<bigint> {
        if (this.processError) {
            try {
                const result = await this.actor.submitBooking(to_candid_Booking_n11(this._uploadFile, this._downloadFile, arg0));
                return result;
            } catch (e) {
                this.processError(e);
                throw new Error("unreachable");
...
function to_candid_record_n12(_uploadFile: (file: ExternalBlob) => Promise<Uint8Array>, _downloadFile: (file: Uint8Array) => Promise<ExternalBlob>, value: {
    ...
    return {
        status: to_candid_BookingStatus_n13(_uploadFile, _downloadFile, value.status),
        date: value.date,
        fullName: value.fullName,
        email: value.email,
        timestamp: value.timestamp,
